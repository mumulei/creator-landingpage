...
## 📄 响应式规则文档 (Responsive Docs)

仓库内已补充三份前端可直接参考的响应式规则文档，位于 `docs/responsive/`：

- `client-web-responsive-rule-detailed.md`
- `client-web-responsive-rule-concise.md`
- `client-web-responsive-rule-summary.md`
...
- **断点与缩放规范 (Breakpoints & Scaling)**：
  - **整体原则**：`1440 / 834 / 402` 分别作为 Desktop / Tablet / Mobile 的标准视觉锚点。前端应先严格还原三套标准稿，再在各自断点区间内部按实际屏宽做动态响应式缩放。
  - **Mobile 移动端 (≤ 640px)**：对齐 Mobile 稿，使用汉堡菜单，安全边距 `--safe-padding` 锁定为 `20px`。
  - **Tablet 平板端 (641px - 1150px)**：对齐 Tablet 稿，保留平板版布局结构，安全边距 `--safe-padding` 锁定为 `40px`。
  - **Desktop 桌面流式缩放端 (1151px - 2559px)**：对齐 Desktop 稿，并保留 CSS 变量流式缩放能力。当前关键变量包括 `--safe-padding: 5.7vw`、`--section-padding-y`、`--title-font-size`、`--scale-grid-gap`、`--scale-card-height` 等。
  - **Ultra-Wide 超 2K 极限端 (≥ 2560px)**：保留现有 2K 锁定逻辑。达到 `2560px` 后，组件停止继续放大，关键尺寸锁定，例如 `--safe-padding: 146px`、`.hero-copy` 和 `.hero-mockup-shell` 最大宽度锁定为 `2268px`，`.hero-mockup-shell__frame` 高度锁定为 `1897px`。超出部分只继续增加左右留白，各 Section 背景仍保持 `100%` 全屏宽度填充。
  - **执行规则**：跨断点时切换布局结构，断点区间内部做动态缩放。不要把 Desktop 布局直接压缩成 Tablet 或 Mobile。
- **Hero 区域自动裁剪与缩放**：
...
更完整的说明请直接查看 `docs/responsive/` 下的三份文档。下面这张表保留为仓库内的快速参考。
...
| **Hero 首屏** | 高度 `100vh` 限制为 `1000px`，大图 Mockup 占位，双按钮横向 | 高度上限变更为 `800px`，Mockup 自动高度，按钮定宽 `184px` | 高度上限 `700px`，Mockup 切到移动端紧凑卡片比例，按钮垂直堆叠并宽度 `100%` |
...
| **Creator Showcase 3D轮播** | 3D 滑轨。卡片宽 `340px`/高 `582px`，中间卡最大，偏移量 `350px`/`650px` | 3D 滑轨。卡片宽 `260px`/高 `445px`，保持中心聚焦，偏移量 `280px`/`540px`（防重叠） | 3D 滑轨。卡片宽 `190px`/高 `325px`，保持中心卡 + 两侧露边，偏移量 `205px`/`395px` |
...
2. **SmoothCorners 平滑圆角降级与兼容**：
   - 按钮与主要卡片容器优先通过 `@lisse/react` (`SmoothCorners` 配合 `smoothing: 0.6`) 生成平滑 Squircle 圆角。
   - **防塌陷降级**: 对于 Showcase 视频滑轨（3D 位移变换容易使 lisse canvas 重绘失败并高度塌陷为正方形）以及 How It Works 特色图容器（内部包含 flex-grow 100% 图片），在 CSS 中允许降级为原生 `border-radius` 做兼容。
